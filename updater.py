...
PROTECTED_NAMES = {
    ".env", ".venv", ".git", ".vs", "__pycache__", "release_state.json",
...
def process_is_running(process_id):
    ...
    Has support for both Windows and Linux
    """
    #Invalid process IDs are treated as not running
    if process_id <=0:
        return False
    ...
    if os.name == "nt":
        try:
            ...
            #tasklist will return information about the process if it exists, IF PID
            #Does not exist, the output normally would say no task.
            process_output = result.stdout.strip()
            if not process_output:
                return False
            if "No tasks are running" in process_output:
                return False
            #If the PID appears in the tasklist, the process is still running
            return str(process_id) in process_output
        #Catches error for process check
        except( subprocess.SubprocessError, OSError):
            #Tasklist can not be checked, Logged error
            return False

    #Linux / Raspberry pie process check
    else:
        try:
            #On unix-like systems will signal 0 to check for whether the process
            #exists without terminating
            ...
            #Process exists, but no permissions granted
            return True
        except OSError:
            return False
...
def parse_arguments():
    ...
    parser.add_argument( "--project-folder", required = True, help = "Path to the bot's project folder.")
...
def main():
    ...
    try:
        ...
        #Creates the update marker
        update_success_file = (project_folder / "update_success.txt")
        update_success_file.write_text(arguments.new_version, encoding="utf-8")
